...
exports.postCartChangeQty = async (req, res, next) => {
  ...
  console.log('exports.exports.postCartChangeQty req.body.reqData',req.body.reqData )//from req.body we get the new qty
  ...
  const product = await Product.findById(prodId)
    ...
    }else{ //user is loggedin => toDo
        const cart = new Cart(req.user.cart);
        newupdatedQuantity = await req.user.changeCartItemQuantity(prodId,newQty);
    }
  ...
  //       }else if (req.session.tempCart){//we already have a cart in our session cast it to document
  //         req.session.tempCart=Cart.hydrate(req.session.tempCart);
  //       }
  //       //ourcart is ready ,now change the qty of item
  //        newupdatedQuantity = await req.session.tempCart.changeCartItemQuantity(prodId,newQty);
